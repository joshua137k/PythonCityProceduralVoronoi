	...
	for line_key in text:
		...
		for item in text[line_key]:
			...
			if start_point and end_point:
				var start_vector = Vector3(start_point[0]/5, 0, start_point[1]/5)
				var end_vector = Vector3(end_point[0]/5, 0, end_point[1]/5)
		...
		primitives_node.draw_line(line_list,3,Color.WHITE)
